...
class AppCpp:
    # Отдельное решение на C++ модулях
    def __init__(self, root):
        self.root = root
        self.root.title("Лабораторная 4.2 - C++")
        self.root.geometry("950x650")

        build_cpp_modules()

        self.backend_name = tk.StringVar(value="C++ dynamic")
        self.backends = {
            "C++ dynamic": CppListModule(DYNAMIC_DLL),
            "C++ STL": CppListModule(STL_DLL),
        }
        self.current = self.backends["C++ dynamic"]
    ...
    def build_ui(self):
        ...
        tk.Label(top, text="Решение: C++").grid(row=0, column=0, sticky="w")
    ...
    def refresh_all(self):
        ...
        self.info_label.config(
            text=f"Текущее решение: {self.backend_name.get()}    Количество элементов: {self.current.count()}    Пустой список: {'да' if self.current.is_empty() else 'нет'}"
        )
